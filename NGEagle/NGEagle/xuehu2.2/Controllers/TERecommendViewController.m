...
@interface TERecommendViewController ()<UITableViewDelegate, UITableViewDataSource>
{
...
@implementation TERecommendViewController
...
- (void)configBaseUI {
    ...
    float x = (SCREEN_WIDTH - 200) / 2;
    for (int i = 0; i < 2; i++) {
        UIButton *btn = [UIButton new];
        [btn addTarget:self action:@selector(selectLevel1:) forControlEvents:UIControlEventTouchUpInside];
        btn.frame = CGRectMake(x + i * 100, 27, 100, 30);
        btn.tag = 100 + i;
        switch (i) {
            case 0:
                [btn setTitle:@"我的推荐" forState:UIControlStateNormal];
                break;
            case 1:
...
- (void)selectLevel1:(UIButton *)btn {
    if (_level == btn.tag - 100) {
        return;
    }
    _level = (int)btn.tag - 100;
    // 刷新数据
    [_tableView.header beginRefreshing];
}
...
- (void)getDataFromServer:(BOOL)isRefresh {
    ...
    if (_level == 0) {
        ...
        if (_type == 0) {
            ...
            _operation = [CourseHelper getMyPublishWeikeList:tempPage num:num type_id:0 subject_id:_subjectId stage_id:0 grade_id:_gradeId kw:nil success:^(id responseObject) {
                CCCourseListModel *model = responseObject;
                [weakSelf reloadData:model isRefresh:isRefresh];
            } fail:^(NSError *error) {
            ...
            _operation = [CourseHelper getMyPublishWeikeAlbumList:tempPage num:num type_id:0 subject_id:_subjectId stage_id:0 grade_id:_gradeId kw:nil success:^(id responseObject) {
                CCCourseListModel *model = responseObject;
                [weakSelf reloadData:model isRefresh:isRefresh];
            } fail:^(NSError *error) {
                [_tableView.header endRefreshing];
...
- (void)reloadData:(CCCourseListModel *)model isRefresh:(BOOL)isRefresh{
