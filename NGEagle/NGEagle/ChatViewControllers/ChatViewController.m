...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, EMCDDeviceManagerDelegate,EMCallManagerDelegate,LocationViewDelegate>
{
...
//  [self setupBarButtonItem];
    ...
    if (!self.isSystemMsg) {
        self.tableView.height = SCREEN_HEIGHT - 64 - 44;
...
- (void)configBaseUI {
    ...
    if (self.isSystemMsg) {
        _titleLabel.text = @"好友、群组消息";
        [_rightButotn setImage:[UIImage imageNamed:@"icon_clear"] forState:UIControlStateNormal];
        
    } else {
        if (self.userInfoDict[@"nick"]) {
            _titleLabel.text = self.userInfoDict[@"nick"];
            
        } else {
            _titleLabel.text = self.chatter;
        }
    }
...
- (void)rightButtonAction {
    ...
    [self removeAllMessages:_rightButotn];
}
...
- (void)showRoomContact:(UIButton *)button {
    ...
    if (_tableView == nil) {
        _tableView = [[UITableView alloc]initWithFrame:
                      CGRectMake(0,
                                 ...
                                 self.view.frame.size.height - self.chatToolBar.frame.size.height - NavigationBar_HEIGHT)];
    ...
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (!self.isSystemMsg) {
                ...
                SystemMsgCell *cell = (SystemMsgCell *)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
                if (cell == nil) {
                    cell = [[[NSBundle mainBundle] loadNibNamed:@"SystemMsgCell" owner:self options:nil] lastObject];
                    cell.backgroundColor = [UIColor clearColor];
                    cell.selectionStyle = UITableViewCellSelectionStyleNone;
                }
                cell.messageModel = model;
...
//        id obj = [self.dataSource objectAtIndex:indexPath.row];
//        MessageModel *model = (MessageModel *)obj;
//        NSDictionary *dataDict = [CCJsonKit getObjectFromJsonString:model.content];
//        int msg_type = [dataDict[@"msg_type"] intValue];
//        int msg_subType = [dataDict[@"msg_subtype"] intValue];
//        
//        
//    }
//}
...
- (void)didChangeFrameToHeight:(CGFloat)toHeight
{
    [UIView animateWithDuration:0.3 animations:^{
        ...
        rect.size.height = self.view.frame.size.height - toHeight - NavigationBar_HEIGHT;
    ...
    if ([messagesArray count] > 0) {
        for (EMMessage *message in messagesArray) {
            ...
            if (tempDate > 60 || tempDate < -60 || (self.chatTagDate == nil)) {
               ...
               NSString *ret = [NSDateUtil formatDate:createDate];
                [formatArray addObject:ret];
...
-(NSMutableArray *)formatMessage:(EMMessage *)message
{
    ...
    if (tempDate > 60 || tempDate < -60 || (self.chatTagDate == nil)) {
        ...
        [ret addObject:[NSDateUtil formatDate:createDate]];
        self.chatTagDate = createDate;
    }
...
-(void)addMessage:(EMMessage *)message
{
    ...
    if (message.messageType != eMessageTypeChat) {
        ...
        @try {
            ...
            if (self.userInfoDict[@"gid"]) {
                [ext setObject:self.userInfoDict[@"gid"] forKey:@"gid"];
            }
            message.ext = ext;
        }
