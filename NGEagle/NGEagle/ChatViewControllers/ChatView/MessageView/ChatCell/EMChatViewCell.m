...
- (void)setMessageModel:(MessageModel *)model
{
    ...
    [_headImageView setImageWithURL:model.headImageURL placeholderImage:[UIImage imageNamed:@"default_head"]];
    
    if (!model.headImageURL) {
        [self setUserInfoByUid:model];
    }
...
- (void)setUserInfoByUid:(MessageModel *)model {

    NSString *uid = model.username;
    ...
    if (user.uid.length > 0) {
        ...
        [_headImageView setImageWithURL:[NSURL URLWithString:user.logo] placeholderImage:[UIImage imageNamed:@"default_head"]];
        _nameLabel.text = user.nick;
        
        model.nickName = user.nick;
        model.headImageURL = [NSURL URLWithString:user.logo];
        ...
        [ChatDataHelper getUserInfoByUid:uid success:^(id responseObject) {
            ...
            if (infoModel.error_code == 0) {
                if (infoModel.user.uid.length > 0) {
                    
                    [UserDao insertUser:infoModel.user];
                    
                    [_headImageView setImageWithURL:[NSURL URLWithString:infoModel.user.logo] placeholderImage:[UIImage imageNamed:@"default_head"]];
                    _nameLabel.text = infoModel.user.nick;
                    
                    model.nickName = infoModel.user.nick;
                    model.headImageURL = [NSURL URLWithString:infoModel.user.logo];
                }
            }
